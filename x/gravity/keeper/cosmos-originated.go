...
import (
	...
	"github.com/unification-com/mainchain/x/gravity/types"
)
...
func (k Keeper) GetCosmosOriginatedDenom(ctx sdk.Context, tokenContract types.EthAddress) (string, bool) {
	...
	iterator := store.Iterator(types.DenomToERC20Key, nil)
	for ; iterator.Valid(); iterator.Next() {
		key := iterator.Key()

		fmt.Println(fmt.Sprintf("key: %s \t value: %s. %s", string(iterator.Key()), bz, string(key[1:])))
